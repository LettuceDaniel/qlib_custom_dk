...
class CoSTEER(Developer[Experiment]):
    def __init__(
        ...
        **kwargs,
        ...
        # Check if settings support auto-versioning (FactorCoSTEERSettings and similar)
        if hasattr(settings, 'auto_knowledge_versioning') and settings.auto_knowledge_versioning:
            if hasattr(settings, 'knowledge_base_folder') and settings.knowledge_base_folder:
                # Use auto-versioning
                self.knowledge_base_path, self.new_knowledge_base_path = resolve_knowledge_base_paths(settings)
                logger.info(f"Auto-versioning: loading from {self.knowledge_base_path}, saving to {self.new_knowledge_base_path}")
            else:
            ...
            # Original behavior for settings without auto-versioning support
            self.knowledge_base_path = (
                Path(settings.knowledge_base_path) if settings.knowledge_base_path is not None else None
            )
            self.new_knowledge_base_path = (
