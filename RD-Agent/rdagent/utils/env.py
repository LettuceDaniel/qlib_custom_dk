    ...
    def _run(
        ...
        **kwargs: dict,
        ...
        with _symlink_ctx(volumes):
            # Setup environment
            if env is None:
                # Copy current environment variables to preserve .env and system settings
                env = os.environ.copy()
...
class CondaConf(LocalConf):
    ...
    def _update_bin_path(self) -> None:
        ...
        conda_path_result = subprocess.run(
            f"/workspace/miniconda/bin/conda run -n {self.conda_env_name} --no-capture-output env | grep '^PATH='",
            capture_output=True,
            text=True,
            shell=True,
        )
        self.bin_path = conda_path_result.stdout.strip().split("=")[1].split(":")[0] if conda_path_result.returncode == 0 else ""
...
class QlibCondaEnv(LocalEnv[QlibCondaConf]):
    def prepare(self) -> None:
        ...
        try:
            ...
            env_exists = self.conf.conda_env_name in envs.stdout
            ...
            # Check if required packages are installed (only when env was just created)
            if not env_exists:
                print(f"[yellow]Checking required packages in '{self.conf.conda_env_name}'...[/yellow]")
                check_result = subprocess.run(
                    f"conda run -n {self.conf.conda_env_name} python -c \"import qlib, catboost, xgboost, tables, torch\"",
                    capture_output=True,
                    shell=True,
                )
            else:
                # Skip dependency check for existing env - assume packages are already installed
                print(f"[green]Using existing env '{self.conf.conda_env_name}', skipping dependency check[/green]")
                check_result = subprocess.run(["true"])
                check_result.returncode = 0

            if check_result.returncode != 0:
                print(f"[yellow]Installing required packages in '{self.conf.conda_env_name}'...[/yellow]")
                subprocess.check_call(
                ...
                print(f"[green]All required packages already installed in '{self.conf.conda_env_name}'[/green]")

            # Download Qlib data if not exists
            from pathlib import Path
            # Check for US data first, then fallback to CN data
            us_data_path = Path("/workspace/qlib_data/us_data")
            cn_data_path = Path("~/.qlib/qlib_data/cn_data").expanduser()

            if us_data_path.exists():
                print(f"[green]Qlib US data already exists at {us_data_path}[/green]")
            elif cn_data_path.exists():
                print(f"[green]Qlib CN data already exists at {cn_data_path}[/green]")
            else:
                print(f"[yellow]Qlib data not found, downloading CN data...[/yellow]")
                print(f"[yellow]This may take a few minutes...[/yellow]")
                subprocess.check_call(
                    f"conda run -n {self.conf.conda_env_name} python -m qlib.run.get_data qlib_data --target_dir ~/.qlib/qlib_data/cn_data --region cn --interval 1d --delete_old False",
                    shell=True,
                )
                print(f"[green]Qlib data downloaded successfully[/green]")
...
class QTDockerEnv(DockerEnv):
    ...
    def prepare(self, *args, **kwargs) -> None:  # type: ignore[no-untyped-def]
        ...
        # Check for US data first, then fallback to CN data
        us_data_path = Path(qlib_data_path) / "qlib_data" / "us_data"
        cn_data_path = Path(qlib_data_path) / "qlib_data" / "cn_data"

        if us_data_path.exists():
            logger.info(f"Qlib US data already exists at {us_data_path}. Download skipped.")
        elif cn_data_path.exists():
            logger.info(f"Qlib CN data already exists at {cn_data_path}. Download skipped.")
        else:
            logger.info("Qlib data not found. Downloading CN data...")
            ...
            logger.info("Qlib CN data downloaded successfully.")
