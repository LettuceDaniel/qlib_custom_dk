...
class QlibModelRunner(CachedRunner[QlibModelExperiment]):
    ...
    def develop(self, exp: QlibModelExperiment) -> QlibModelExperiment:
        ...
        if run_id is not None:
            env_to_use["RDAGENT_RUN_ID"] = run_id
        if mbps.test_end is not None and mbps.test_end != "":
        ...
        if training_hyperparameters:
            # Handle case where training_hyperparameters is a JSON string
            ...
            env_to_use.update(
                {
                    ...
                    "early_stop": str(training_hyperparameters.get("early_stop", 7)),
                    "batch_size": str(training_hyperparameters.get("batch_size", 512)),
                    "weight_decay": str(training_hyperparameters.get("weight_decay", 1e-5)),
                }
            )
        ...
        if exp.sub_tasks[0].model_type == "TimeSeries":
            if exist_sota_factor_exp:
                ...
                env_to_use.update({"dataset_cls": "TSDatasetH", "step_len": str(20), "num_timesteps": str(20)})
                result, stdout = exp.experiment_workspace.execute(
