...
from rdagent.scenarios.qlib.experiment.utils import (
    get_data_statistics_for_rag,
    get_hyperparameter_guidelines,
)
...
class QlibModelHypothesisGen(ModelHypothesisGen):
    ...
    def prepare_context(self, trace: Trace) -> Tuple[dict, bool]:
        ...
        # Get dynamic data statistics and hyperparameter guidelines
        data_rag = get_data_statistics_for_rag() + "\n\n" + get_hyperparameter_guidelines()
...
class QlibModelHypothesis2Experiment(ModelHypothesis2Experiment):
    ...
    def convert_response(self, response: str, hypothesis: Hypothesis, trace: Trace) -> ModelExperiment:
        ...
        for model_name in response_dict:
            ...
            # Convert dict to JSON string for consistency with ModelTask type
            if isinstance(hyperparameters, dict):
                hyperparameters = json.dumps(hyperparameters)
            if isinstance(training_hyperparameters, dict):
                training_hyperparameters = json.dumps(training_hyperparameters)
            tasks.append(
                ModelTask(
                    name=model_name,
