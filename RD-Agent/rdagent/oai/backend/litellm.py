...
class LiteLLMSettings(LLMSettings):
    model_config = {"env_prefix": "LITELLM_"}
...
class LiteLLMAPIBackend(APIBackend):
    ...
    def _create_embedding_inner_function(self, input_content_list: list[str]) -> list[list[float]]:
        ...
        # Prepare embedding kwargs with api_key and api_base if available
        embedding_kwargs = {
            "model": model_name,
            "input": input_content_list,
            "encoding_format": "float",  # Explicitly set encoding_format for DashScope compatibility
        }
    ...
    def get_complete_kwargs(self) -> CompleteKwargs:
        ...
        if LITELLM_SETTINGS.chat_model_map:
            logger.info(f"[DEBUG] logger._tag = '{logger._tag}'", tag="llm_messages")
            logger.info(f"[DEBUG] chat_model_map = {LITELLM_SETTINGS.chat_model_map}", tag="llm_messages")
            for t, mc in LITELLM_SETTINGS.chat_model_map.items():
                if t in logger._tag:
                    logger.info(f"[DEBUG] Found matching tag: '{t}' in '{logger._tag}'", tag="llm_messages")
                    model = mc["model"]
    ...
    def _create_chat_completion_inner_function(  # type: ignore[no-untyped-def] # noqa: C901, PLR0912, PLR0915
        ...
        **kwargs,
        ...
        logger.info(f"[DEBUG] Using model: {model}, logger._tag = '{logger._tag}'", tag="llm_messages")

        # Prepare completion kwargs with api_key and api_base if available
        completion_kwargs = {
            "messages": messages,
        ...
        if LITELLM_SETTINGS.openai_api_key:
            completion_kwargs["api_key"] = LITELLM_SETTINGS.openai_api_key
        if LITELLM_SETTINGS.openai_api_base:
            completion_kwargs["api_base"] = LITELLM_SETTINGS.openai_api_base
