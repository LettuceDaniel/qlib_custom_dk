...
class APIBackend(ABC):
    ...
    def create_embedding(self, input_content: str | list[str], *args, **kwargs) -> list[float] | list[list[float]]:  # type: ignore[no-untyped-def]
        ...
        # Return dummy embeddings when embedding is disabled
        if os.environ.get("DISABLE_EMBEDDING", "").lower() in ("true", "1", "yes"):
            input_content_list = [input_content] if isinstance(input_content, str) else input_content
            dummy_embedding = [0.0] * 1536  # OpenAI ada-002 default dimension
            logger.info(f"Embedding disabled, returning dummy zeros for {len(input_content_list)} items")
            return dummy_embedding if isinstance(input_content, str) else [dummy_embedding] * len(input_content_list)

        input_content_list = [input_content] if isinstance(input_content, str) else input_content

        # Handle batch size limitations by chunking large requests
        # Some providers (e.g., Qwen/DashScope) have a max batch size of 10
        max_batch_size = 10
        if len(input_content_list) > max_batch_size:
            logger.info(
                f"Input list has {len(input_content_list)} items, exceeding max batch size of {max_batch_size}. "
                f"Splitting into {len(input_content_list) // max_batch_size + 1} batches."
            )
            all_embeddings = []
            for i in range(0, len(input_content_list), max_batch_size):
                batch = input_content_list[i:i + max_batch_size]
                batch_embeddings = self._try_create_chat_completion_or_embedding(  # type: ignore[misc]
                    input_content_list=batch,
                    embedding=True,
                    *args,
                    **kwargs,
                )
